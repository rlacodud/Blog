...
$('.back-color').on('click', function () {
  ...
  reset();
  // --------------------------------------------------------------------------------------추가된 부분
  $('.back-color').removeClass('clicked');
  // --------------------------------------------------------------------------------------끝
...
$('#text').on('touchstart', function (e) {
  if (isColor) {
    ...
    // '가나다라'라는 기본 text 삽입
    targetText.innerHTML = '가나다라';
    // drag-area-text의 자식 요소로 삽입
    target.appendChild(targetText);

    // 삭제 버튼이 될 div 생성
    const textRemoveButton = document.createElement("div");
    // remove라는 class명 부여
    ...
    $('.drag-area-text').off('touchstart').on('touchstart', function (e) {
      ...
      // remove 버튼 클릭 시
      $(this).children('.drag-text').children('.remove').off('touchstart').on('touchstart', function () {
        // 현재 선택된 element 제거
        thisDragText.remove();
      })
    })
