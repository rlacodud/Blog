...
let dragEl = null;
let target = null;
let removeButtonContainer = null
...
// 각 icon 클릭 시 실행
$('.icon-button').on('click touchstart', function (e) {
  // isImage가 true일 때 실행
  if (isImage) {
    // isDeco를 true로 변경
    isDeco = true;
    ...
    $('.drag-area').off('click touchstart').on('click touchstart', function (e) {
      // 이벤트 버블링 방지
      e.stopPropagation();
      // reset 호출
      reset();
      ...
      let thisDrag = $(this);
      // remove 버튼 클릭 시
      $(this).children('.drag').children('.remove').off('touchstart').on('touchstart', function () {
...
// Text----------------------------------------------------------------
// Text 생성 버튼 클릭 시
$('#text').off('click touchstart').on('click touchstart', function (e) {
...
$('.input-area').on('keyup', function () {
  ...
  $('.drag-area-text.on .drag-text .text').text(changeText);
